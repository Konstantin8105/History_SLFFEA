/* This program calculates the eigenvalues of the matrix system:
   ...
   a brick.  Note that implicit in this algorithm is the assumption
...
int brMassPassemble(int *, double *, int *, double *, MATL *, double *, double *);

int QR_check( double *, double *, double *, int, int );

int brLanczos(double *K, BOUND bc, int *connect, double *coord, EIGEN *eigen, 
...
*/
	    ...
	    check = brMassPassemble(connect, coord, el_matl, M, matl,
		fdum_vector_out, fdum_vector_in);
            if(!check) printf( " Problems with brMassPassemble \n");
...
*/
		...
		check = brConjGrad( K, bc, connect, coord, el_matl, fdum_vector_in,

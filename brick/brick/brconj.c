/*
    ...
    element program which does analysis on a brick.  The first function
...
int brConjPassemble(double *A, int *connect, double *coord, int *el_matl, MATL *matl,
...
int brConjGrad(double *A, BOUND bc, int *connect, double *coord, int *el_matl,
...
/* This function does memory allocation and uses the conjugate gradient
   ...
   displacements.  It also makes the call to brConjPassemble to get the
   product of [A]*[p].
...
/* Using Conjugate gradient method to find displacements */
	...
	while(fdum2 > tolerance && counter < iteration_max )
		...
		printf( "\n %3d %16.8e\n",counter, fdum2);
		check = brConjPassemble( A, connect, coord, el_matl, matl, P_global, p);

...
int trConjGrad(double *, BOUND , int *, double *, int *, double *, double *, MATL *,
	double *);
...
int trMassemble(int *, double *, int *, int *, double *, MATL *);

int trKassemble(double *, int *, double *, int *, double *, int *, int *,
	double *, int *, MATL *, double *, STRAIN *, STRAIN *, STRESS *,
	STRESS *, double *, double *);

int diag( int *, int *, int, int, int, int);

int formlm( int *, int *, int *, int, int, int );

int trformid( BOUND, int *);
...
int analysis_flag, dof, modal_flag, neqn, nmat, nmode, numel, numnp, plane_stress_flag,
	sof, stress_read_flag;
int gauss_stress_flag = 1;
...
int main(int argc, char** argv)
{
		...
		sofmSTRESS_node, sofmXYI, ptr_inc;
        MATL *matl;
        double *mem_double;
...
/* If modal analysis is desired, allocate for the eigens */
	...
	check = trreader( bc, connect, coord, el_matl, force, matl, name, o1,
        ...
        check = trformid( bc, id );
        if(!check) printf( " Problems with trformid \n");
/*
...
*/
		check = trMassemble(connect, coord, el_matl, id, mass, matl);

...
/* Using Conjugate gradient method to find displacements */
	...
	while(fdum2 > tolerance && counter < iteration_max )
	{
		...
		printf( "\n %3d %16.8e\n",counter, fdum2);
		check = trConjPassemble( A, connect, coord, el_matl, matl, P_global, p);
		if(!check) printf( " Problems with trConjPassemble \n");
		check = trBoundary (P_global, bc);
		if(!check) printf( " Problems with trBoundary \n");
...
/*
	check = trConjPassemble( A, connect, coord, el_matl, matl, P_global, U);
	if(!check) printf( " Problems with trConjPassemble \n");

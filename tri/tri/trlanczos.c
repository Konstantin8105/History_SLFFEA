/* This program calculates the eigenvalues of the matrix system:
   ...
   a 4 node trianglerilateral element.  Note that implicit in this algorithm is
...
int trMassPassemble(int *, double *, int *, double *, MATL *, double *, double *);

int QR_check( double *, double *, double *, int, int );

int trLanczos(double *K, BOUND bc, int *connect, double *coord, EIGEN *eigen,
	int *el_matl, int *id, int *idiag, double *K_diag,  double *M, MATL *matl,
...
*/
	    ...
	    check = trMassPassemble(connect, coord, el_matl, M, matl,
		fdum_vector_out, fdum_vector_in);
            if(!check) printf( " Problems with trMassPassemble \n");
...
*/
		...
		check = trConjGrad( K, bc, connect, coord, el_matl, fdum_vector_in,
			K_diag, matl, fdum_vector_out);

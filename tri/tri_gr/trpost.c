/*
    ...
    element mesh for triangle elements.
...
int trMemory_gr( ISTRAIN **, ISTRESS **, int );

int trMemory( double **, int, int **, int, MATL **, int , XYI **, int,
...
void trMeshKeys( unsigned char , int , int  );
...
void trControlMouse(int , int , int , int );

void ControlReshape(int , int );

void trControlDisplay(void);
...
int main(int argc, char** argv)
{
	...
	int sofmi, sofmf, sofmSTRESS, sofmSTRESS_node, sofmISTRESS,
		sofmSTRAIN, sofmXYI, sofmXYF, ptr_inc;
...
/* For the ISTRESS */
	...
	check = trMemory( &mem_double, sofmf, &mem_int, sofmi, &matl, nmat,
		&mem_XYI, sofmXYI, &strain, &strain_node, &stress, &stress_node, 
		sofmSTRESS, sofmSTRESS_node );
	if(!check) printf( " Problems with trMemory \n");

	check = trMemory_gr( &strain_color, &stress_color, sofmISTRESS );
	if(!check) printf( " Problems with trMemory_gr \n");
...
/*
   This is allocated seperately from trMemory_gr because we need to know the
   number of force vectors read from trreader and stored in bc.num_force[0].
*/

	check = trMemory2_gr( &mem_XYF, sofmXYF );
	if(!check) printf( " Problems with trMemory2_gr \n");
...
/* Initialize the Control Panel */
        ...
        ControlInit();
	trMenu();
        glutDisplayFunc(trControlDisplay);
        glutReshapeFunc(ControlReshape);

        glutMouseFunc(trControlMouse);

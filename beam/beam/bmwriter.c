...
extern int standard_flag, gauss_stress_flag;
...
/* Every output file is named after the input file with
   a ".obm" extension */
	...
	if(!ccheck) printf( " Problems with strncpy \n");
		...
		matl[i].nu, matl[i].rho, matl[i].area, matl[i].Iy, matl[i].Iz);
        }
	...
	fprintf( o3, "local stress xx and moment xx,yy,zz\n");
	for( i = 0; i < numel; ++i )
	{
	   for( j = 0; j < num_int; ++j )
	   {
	...
	fprintf( o3, " -10 \n");
	fprintf( o3, "element no. and %5s pt. no. with ", stress_type);
	fprintf( o3, "local strain xx and curvature xx,yy,zz\n");
	for( i = 0; i < numel; ++i )
	{
	   for( j = 0; j < num_int; ++j )
	   {

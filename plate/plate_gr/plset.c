...
/* Assign colors for Uz */
               *(U_color + ndof*i) = 0;
               if(  *(U + ndof*i) > Uz_div[1] )
               {
                  *(U_color + ndof*i) = 1;
                  if(  *(U + ndof*i) > Uz_div[2] )
                  {
               	     *(U_color + ndof*i) = 2;
                     if(  *(U + ndof*i) > Uz_div[3] )
                     {
                        *(U_color + ndof*i) = 3;
                        if(  *(U + ndof*i) > Uz_div[4] )
                        {
                           *(U_color + ndof*i) = 4;
                           if(  *(U + ndof*i) > Uz_div[5] )
                           {
                              *(U_color + ndof*i) = 5;
                              if(  *(U + ndof*i) > Uz_div[6] )
                              {
                                 *(U_color + ndof*i) = 6;
                                 if(  *(U + ndof*i) > Uz_div[7] )
                                 {
                                    *(U_color + ndof*i) = 7;
                                 }
                              }
...
/* Assign colors for Uphi_x */
               *(U_color + ndof*i + 1) = 0;
               if(  *(U + ndof*i + 1) > Uphi_x_div[1] )
               {
                  *(U_color + ndof*i + 1) = 1;
                  if(  *(U + ndof*i + 1) > Uphi_x_div[2] )
                  {
               	     *(U_color + ndof*i + 1) = 2;
                     if(  *(U + ndof*i + 1) > Uphi_x_div[3] )
                     {
                        *(U_color + ndof*i + 1) = 3;
                        if(  *(U + ndof*i + 1) > Uphi_x_div[4] )
                        {
                           *(U_color + ndof*i + 1) = 4;
                           if(  *(U + ndof*i + 1) > Uphi_x_div[5] )
                           {
                              *(U_color + ndof*i + 1) = 5;
                              if(  *(U + ndof*i + 1) > Uphi_x_div[6] )
                              {
                                 *(U_color + ndof*i + 1) = 6;
                                 if(  *(U + ndof*i + 1) > Uphi_x_div[7] )
                                 {
                                    *(U_color + ndof*i + 1) = 7;
                                 }
                              }
...
/* Assign colors for Uphi_y */
               *(U_color + ndof*i + 2) = 0;
               if(  *(U + ndof*i + 2) > Uphi_y_div[1] )
               {
                  *(U_color + ndof*i + 2) = 1;
                  if(  *(U + ndof*i + 2) > Uphi_y_div[2] )
                  {
               	     *(U_color + ndof*i + 2) = 2;
                     if(  *(U + ndof*i + 2) > Uphi_y_div[3] )
                     {
                        *(U_color + ndof*i + 2) = 3;
                        if(  *(U + ndof*i + 2) > Uphi_y_div[4] )
                        {
                           *(U_color + ndof*i + 2) = 4;
                           if(  *(U + ndof*i + 2) > Uphi_y_div[5] )
                           {
                              *(U_color + ndof*i + 2) = 5;
                              if(  *(U + ndof*i + 2) > Uphi_y_div[6] )
                              {
                                 *(U_color + ndof*i + 2) = 6;
                                 if(  *(U + ndof*i + 2) > Uphi_y_div[7] )
                                 {
                                    *(U_color + ndof*i + 2) = 7;
                                 }
                              }
...
/* Calculate the graphical force vectors for display */
		...
		force_vec0[i].phix =
		   *(force + ndof*bc.force[i]+1)*2.0*AxisLength_max/moment_vec_length;
		force_vec0[i].phiy =
		   *(force + ndof*bc.force[i] + 2)*2.0*AxisLength_max/moment_vec_length;
	        /*printf(" force %d %10.5e %10.5e %10.5e %10.5e %10.5e\n", i, 
		   *(force + ndof*bc.force[i]), *(force + ndof*bc.force[i] + 1),
		   force_vec0[i].z, force_vec0[i].phix, force_vec0[i].phiy);*/
        }

...
extern double Uz_div[boxnumber+1], Uphi_x_div[boxnumber+1], Uphi_y_div[boxnumber+1];
...
/* Update Coordinates */
				for ( i = 0; i < numnp; ++i )
				{
				   *(coord + nsd*i) = *(coord0+nsd*i) +
					*(U+ndof*i)*(*(U+ndof*i+2))*amplify_factor;
				   *(coord + nsd*i + 1) = *(coord0+nsd*i+1) -
					*(U+ndof*i)*(*(U+ndof*i+1))*amplify_factor;
				   *(zcoord + i) = *(U + ndof*i)*amplify_factor;
				}
...
/* Angle Y*/
	...
	for(i = 29; i < rowdim ; ++i)
	{
    	...
    	switch (color_choice) {
			...
			fpointz = *(zcoord + node_choice)*coord_rescale;
			if(!After_flag)
			{

/* This program calculates the eigenvalues of the matrix system:
   ...
   a 4 node quadrilateral element.  Note that implicit in this algorithm is
   the assumption that [M] is positive definite.
...
int plMassPassemble(int *, double *, int *, double *, double *, MATL *, double *,
	double *);

int plMassPassemble_triangle(int *, double *, int *, double *, double *, MATL *,
	double *, double *);

int QR_check( double *, double *, double *, int, int );

extern int dof, neqn, sof, flag_quad_element;
...
int plLanczos(double *K, BOUND bc, int *connect, double *coord, EIGEN *eigen,
	int *el_matl, int *id, int *idiag, double *K_diag, double *local_xyz, double *M,
	MATL *matl, double *ritz, int num_eigen)
{
...
*/
	    ...
	    {
/* Triangle elements */
		check = plMassPassemble_triangle(connect, coord, el_matl,
		    local_xyz, M, matl, fdum_vector_out, fdum_vector_in);
		if(!check) printf( " Problems with plMassPassemble_triangle\n");
	    }
	    else
	    {
/* Quad elements */
		check = plMassPassemble(connect, coord, el_matl, local_xyz,
		    M, matl, fdum_vector_out, fdum_vector_in);
		if(!check) printf( " Problems with plMassPassemble \n");
	    }
...
*/
		...
		check = plConjGrad( K, bc, connect, coord, el_matl, fdum_vector_in,
			K_diag, local_xyz, matl, fdum_vector_out);

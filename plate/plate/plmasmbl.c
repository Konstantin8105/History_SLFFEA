...
extern SH shg, shl;
extern double w[num_int4+1], *Area0;
...
int plMassemble(int *connect, double *coord, int *el_matl, int *id, double *local_xyz, 
...
#if 0
		for( i1 = 0; i1 < num_int4; ++i1 )
		{
		    for( i2 = 0; i2 < npel4; ++i2 )
		    {
			printf("%10.6f ",*(shl+npel4*(nsd2+1)*i1 + npel4*(nsd2) + i2));
		    }
		    printf(" \n");
		}
		printf(" \n");
...
*/
		...
		/*for( i1 = 0; i1 < neqel24; ++i1 )
		{
		   ...
		   {
			printf("%12.5e ",*(M_temp+neqel24*i1+i2));
		   }
		   printf("\n");
		}
		printf("bbbb %d %d\n", neqel24, k);*/
...
/* Creating the diagonal lumped mass Matrix */
			...
			for( i3 = 0; i3 < neqel24; ++i3 )
			{
			    *(mass_el+i2) += *(M_el+neqel24*i2+i3);
			}
			/*printf("%9.6f\n\n",*(mass_el+i2));*/
			fdum += *(mass_el+i2);
		    }   
...
/* Storing all the element mass matrices */
		    ...
		    for( j = 0; j < neqlsq576; ++j )
		    {   
			*(mass + neqlsq576*k + j) = *(M_el + j);
		    }
		}
	}

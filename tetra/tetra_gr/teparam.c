/*
    ...
    the FEM GUI for tetrahedral elements.
...
int teparameter( double *coord, STRAIN *strain_node, STRESS *stress_node, double *U)
{
        int i, j, check;
...
/* Print the above data in the file "teview.dat" */

	fprintf( tedata, "                            node\n");
	fprintf( tedata, "                          min  max       min            max\n");
	fprintf( tedata,"displacement Ux        %5d %5d   %14.6e %14.6e\n", node_Ux_min,
		node_Ux_max, min_Ux, max_Ux);
	fprintf( tedata,"displacement Uy        %5d %5d   %14.6e %14.6e\n", node_Uy_min,
		node_Uy_max, min_Uy, max_Uy);
	fprintf( tedata,"displacement Uz        %5d %5d   %14.6e %14.6e\n", node_Uz_min,
		node_Uz_max, min_Uz, max_Uz);
	fprintf( tedata,"\n");
	fprintf( tedata, "                            node\n");
	fprintf( tedata, "                        min       max         min           max\n");
	fprintf( tedata,"stress xx            %5d     %5d   %14.6e %14.6e\n", min_stress_node.xx,
		max_stress_node.xx, min_stress.xx, max_stress.xx);
	fprintf( tedata,"stress yy            %5d     %5d   %14.6e %14.6e\n", min_stress_node.yy,
		max_stress_node.yy, min_stress.yy, max_stress.yy);
	fprintf( tedata,"stress zz            %5d     %5d   %14.6e %14.6e\n", min_stress_node.zz,
		max_stress_node.zz, min_stress.zz, max_stress.zz);
	fprintf( tedata,"stress xy            %5d     %5d   %14.6e %14.6e\n", min_stress_node.xy,
		max_stress_node.xy, min_stress.xy, max_stress.xy);
	fprintf( tedata,"stress zx            %5d     %5d   %14.6e %14.6e\n", min_stress_node.zx,
		max_stress_node.zx, min_stress.zx, max_stress.zx);
	fprintf( tedata,"stress yz            %5d     %5d   %14.6e %14.6e\n", min_stress_node.yz,
		max_stress_node.yz, min_stress.yz, max_stress.yz);
	fprintf( tedata,"stress I             %5d     %5d   %14.6e %14.6e\n", min_stress_node.I,
		max_stress_node.I, min_stress.I, max_stress.I);
	fprintf( tedata,"stress II            %5d     %5d   %14.6e %14.6e\n", min_stress_node.II,
		max_stress_node.II, min_stress.II, max_stress.II);
	fprintf( tedata,"stress III           %5d     %5d   %14.6e %14.6e\n", min_stress_node.III,
		max_stress_node.III, min_stress.III, max_stress.III);
	fprintf( tedata,"\n");
	fprintf( tedata,"strain xx            %5d     %5d   %14.6e %14.6e\n", min_strain_node.xx,
		max_strain_node.xx, min_strain.xx, max_strain.xx);
	fprintf( tedata,"strain yy            %5d     %5d   %14.6e %14.6e\n", min_strain_node.yy,
		max_strain_node.yy, min_strain.yy, max_strain.yy);
	fprintf( tedata,"strain zz            %5d     %5d   %14.6e %14.6e\n", min_strain_node.zz,
		max_strain_node.zz, min_strain.zz, max_strain.zz);
	fprintf( tedata,"strain xy            %5d     %5d   %14.6e %14.6e\n", min_strain_node.xy,
		max_strain_node.xy, min_strain.xy, max_strain.xy);
	fprintf( tedata,"strain zx            %5d     %5d   %14.6e %14.6e\n", min_strain_node.zx,
		max_strain_node.zx, min_strain.zx, max_strain.zx);
	fprintf( tedata,"strain yz            %5d     %5d   %14.6e %14.6e\n", min_strain_node.yz,
		max_strain_node.yz, min_strain.yz, max_strain.yz);
	fprintf( tedata,"strain I             %5d     %5d   %14.6e %14.6e\n", min_strain_node.I,
		max_strain_node.I, min_strain.I, max_strain.I);
	fprintf( tedata,"strain II            %5d     %5d   %14.6e %14.6e\n", min_strain_node.II,
		max_strain_node.II, min_strain.II, max_strain.II);
	fprintf( tedata,"strain III           %5d     %5d   %14.6e %14.6e\n", min_strain_node.III,
		max_strain_node.III, min_strain.III, max_strain.III);
	fprintf( tedata,"\n");
	fprintf( tedata,"Orthographic viewport parameters(right, left, top, bootom, near, far)\n ");
	fprintf( tedata,"%14.6e %14.6e %14.6e %14.6e %14.6e %14.6e\n", ortho_right, ortho_left,
		ortho_top, ortho_bottom, near, 1000.0);
	fprintf( tedata,"Perspective viewport parameters( mesh width and height)\n ");
	fprintf( tedata,"%6d %6d\n", mesh_width, mesh_height);
	fprintf( tedata,"Step sizes in x, y, z\n ");
	fprintf( tedata,"%14.6e %14.6e %14.6e\n",step_sizex, step_sizey, step_sizez);
	fprintf( tedata,"Amplification size\n ");
	fprintf( tedata,"%14.6e\n",amplify_step0);

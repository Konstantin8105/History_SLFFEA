/*
    ...
    program which does analysis on a wedge element. 
...
int wewriter ( BOUND bc, int *connect, double *coord, int *el_matl, double *force,
...
/* Every output file is named after the input file with
   a ".owe" extension */
	...
	ccheck = strncpy(out+name_length, ".owe", 4);
	if(!ccheck) printf( " Problems with strncpy \n");

	o3 = fopen( out,"w" );

	fprintf( o3, "   numel numnp nmat nmode (This is for the wedge mesh file: %s)\n", name);
...
/* Open stress data output file */
		...
		ccheck = strncpy(stress_dat+name_length, ".str.owe", 8);
		if(!ccheck) printf( " Problems with strncpy \n");

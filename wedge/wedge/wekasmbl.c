/*
    ...
    element program which does analysis on a wedge element.
...
int weKassemble(double *A, int *connect, int *connect_surf, double *coord, int *el_matl,
...
/* Assembly of the westress_shg matrix for each nodal point */

/*
   westress_shg calculates shg at the nodes.  It is more efficient than weshg
   ...
   is calculated at the Gauss points, weshg is used.

			    check=westress_shg(det, k, shl_node2, shg_node, coord_el_trans);
			    check=weshg(det, k, shl_node, shg_node, coord_el_trans);
*/
...
/* Calculate shg at nodal point */
			    check=westress_shg(det, k, shl_node2, shg_node, coord_el_trans);
			    if(!check) printf( "Problems with westress \n");
			}
...
/* Calculate B matrix at nodal point */
				check = wedgeB((shg_node+npel*(nsd+1)*j),B);
				if(!check) printf( "Problems with wedgeB \n");
			   }

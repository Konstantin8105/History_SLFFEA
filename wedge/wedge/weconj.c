...
/* Using Conjugate gradient method to find displacements */
	...
	while(fdum2 > tolerance && counter < iteration_max )
		...
		printf( "\n %3d %16.8e\n",counter, fdum2);
		check = weConjPassemble( A, connect, coord, el_matl, matl, P_global, p);
		if(!check) printf( " Problems with weConjPassemble \n");
		check = weBoundary (P_global, bc);
		if(!check) printf( " Problems with weBoundary \n");
...
/*
	check = weConjPassemble( A, connect, coord, el_matl, matl, P_global, U);
	if(!check) printf( " Problems with weConjPassemble \n");

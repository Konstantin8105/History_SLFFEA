/* This program calculates the eigenvalues of the matrix system:
   ...
   a wedge element.  Note that implicit in this algorithm is the assumption
...
int weMassPassemble(int *, double *, int *, double *, MATL *, double *, double *);

int QR_check( double *, double *, double *, int, int );

int weLanczos(double *K, BOUND bc, int *connect, double *coord, EIGEN *eigen, 
...
*/
	    ...
	    check = weMassPassemble(connect, coord, el_matl, M, matl,
		fdum_vector_out, fdum_vector_in);
	    if(!check) printf( " Problems with weMassPassemble \n");
...
*/
		...
		check = weConjGrad( K, bc, connect, coord, el_matl, fdum_vector_in,

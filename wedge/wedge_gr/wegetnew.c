...
int weReGetMemory2_gr( XYZF **, int );
...
int weReGetMemory_gr( ISTRAIN **, ISTRESS **, NORM **, int, int );

int weReGetMemory( double **, int, int **, int, MATL **, int , XYZI **, int,
...
/* For the NORMS */
	...
	check = weReGetMemory( &mem_double, sofmf, &mem_int, sofmi, &matl, nmat,
		&mem_XYZI, sofmXYZI, &mem_SDIM, sofmSDIM, &strain, &stress, sofmSTRESS );
	if(!check) printf( " Problems with weReGetMemory \n");

	check = weReGetMemory_gr( &strain_color, &stress_color, &mem_NORM, sofmISTRESS,
		sofmNORM );
	if(!check) printf( " Problems with weReGetMemory_gr \n");
...
/*
   This is allocated seperately from weReGetMemory_gr because we need to know the
   number of force vectors read from wereader and stored in bc.num_force[0].
*/

	check = weReGetMemory2_gr( &mem_XYZF, sofmXYZF );
	if(!check) printf( " Problems with weReGetMemory2_gr \n");

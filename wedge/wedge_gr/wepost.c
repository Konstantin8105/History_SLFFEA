...
void weforce_vectors0(int , BOUND , double *, XYZF *);

void wedisp_vectors0(int , BOUND , double *);

void agvMakeAxesList(GLuint);

int weset(BOUND , int *, double *, XYZF *, SDIM *, ISTRAIN *,
	SDIM *, ISTRESS *, double *, int *);

int weparameter( double *, SDIM *, SDIM *, double *);

int wenormal_vectors (int *, double *, NORM * );

int weMemory2_gr( XYZF **, int );

int weConnectSurfreader( int *, int *, char *);

int wereader_gr( FILE *, SDIM *, SDIM *);

int wereader( BOUND , int *, double *, int *, double *, MATL *, char *,
	FILE *, STRESS *, SDIM *, double *);

int weMemory_gr( ISTRAIN **, ISTRESS **, NORM **, int, int );
...
void weMeshKeys( unsigned char , int , int  );
...
void weControlMouse(int , int , int , int );

void ControlReshape(int , int );

void weControlDisplay(void);
...
/* Initialize filenames */
	...
	memset(owe_exten,0,4*sizeof(char));

	ccheck = strncpy(owe_exten,".owe",4);
...
/* Begin exmaining and checking for the existence of data files */
	...
	check = filecheck( name, name2, &o1, &o2, &o3, owe_exten );
...
/* For the NORMS */
	...
	check = weMemory_gr( &strain_color, &stress_color, &mem_NORM, sofmISTRESS,
		sofmNORM );
	if(!check) printf( " Problems with weMemory_gr \n");
...
/* If there is no input file, then set coord0 to coord */
	...
	if( post_flag )
	{
		...
		stress_read_flag = 0;
        	...
        	check = wereader_gr( o3, strain_node, stress_node);
        	if(!check) printf( " Problems with wereader_gr \n");
	}
	if( input_flag )
	{
	...
	check = weConnectSurfreader( connecter, el_matl, name);
	if(!check) printf( " Problems with weConnectSurfreader \n");
...
/*
   This is allocated seperately from weMemory_gr because we need to know the
   number of force vectors read from wereader and stored in bc.num_force[0].
*/

	check = weMemory2_gr( &mem_XYZF, sofmXYZF );
	if(!check) printf( " Problems with weMemory2_gr \n");
...
/* In mesh viewer, search for extreme values of nodal points, displacements
   ...
   assignments.  Also initialize variables */
	...
	check = weparameter( coord, strain_node, stress_node, U);
        if(!check) printf( " Problems with weparameter \n");
...
/* Rescale undeformed coordinates */
	...
	check = weset( bc, connecter, force, force_vec0, strain_node,
		strain_color, stress_node, stress_color, U, U_color);
        if(!check) printf( " Problems with weset \n");
...
/* create display list for displacement and force grapics vectors
   on undeformed mesh*/
  	    ...
  	    wedisp_vectors0(DispList, bc, coord0);
		...
		force_vec[i].z = fpointz - force_vec0[i].z;
  	    ...
  	    weforce_vectors0(ForceList, bc, coord0, force_vec);
...
/* Initialize the Control Panel */
        ...
        ControlInit();
	weMenu();
        glutDisplayFunc(weControlDisplay);
        glutReshapeFunc(ControlReshape);

        glutMouseFunc(weControlMouse);

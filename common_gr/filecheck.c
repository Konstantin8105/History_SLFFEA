...
/* Every post processor file is named after the input file with
   a 4 character extension consisting of a period and the three
   letters identifying the element type */

	check = strncmp((name2 + name2_length -4), file_exten,4);
...
/* Look for standard input file */
		...
		input_name_length = name2_length - 4;
...
/* Look for modal analysis ouput file if nmode != 0 */
		if(nmode)
		{
			...
			ccheck = strncpy((out+input_name_length+dum), file_exten, 4);
			if(!ccheck) printf( " Problems with strncpy \n");
...
/* If mode_choice = 0, Look for standard output file */
			ccheck = strncpy(out+input_name_length, file_exten, 4);
			if(!ccheck) printf( " Problems with strncpy \n");

/*
    ...
    for brick, brick2, tetrahedron, and wedge elements.
...
#if BRICK1 || BRICK2 || WEDGE1 || WEDGE2
...
extern GLuint AxesList, DispList, ForceList;
extern int chooser, chooser_flag;
...
#if BRICK1
int set(BOUND , int *, double *, XYZF *, SDIM *, ISTRAIN *, SDIM *,
	ISTRESS *, double *, int *);

int brGetNewMesh(void);
...
void MeshKeys( unsigned char key, int x, int y )
{
...
/* Reset the translation */
	    ...
	    case 'g':
		check = brGetNewMesh();
		if(!check) printf( " Problems with brGetNewMesh\n");
		break;
	    case 'h':
		ReGetparameter();
	    ...
	    case 'g':
		check = teGetNewMesh();
		if(!check) printf( " Problems with teGetNewMesh\n");
		break;
	    case 'h':
		ReGetparameter();
	    ...
	    case 'g':
		check = weGetNewMesh();
		if(!check) printf( " Problems with weGetNewMesh\n");
		break;
	    case 'h':
		ReGetparameter();
		check = set( bc, connecter, force, force_vec0, strain_node,

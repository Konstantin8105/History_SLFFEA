...
/* Calculate local deformed coordinates */
			...
			if( *(coord_el+3*j + 2) > cross_sec_in_out)
				After_element_draw_flag = 0;
			if( *(coord_el+3*npell+3*j) > cross_sec_left_right)
				After_element_draw_flag = 0;
			if( *(coord_el+3*npell+3*j + 1) > cross_sec_up_down)
				After_element_draw_flag = 0;
			if( *(coord_el+3*npell+3*j + 2) > cross_sec_in_out)
				After_element_draw_flag = 0;
		    }
...
/* Calculate local undeformed coordinates */
			...
			if( *(coord0_el+3*j + 2) > cross_sec_in_out)
				Before_element_draw_flag = 0;
			if( *(coord0_el+3*npell+3*j) > cross_sec_left_right)
				Before_element_draw_flag = 0;
			if( *(coord0_el+3*npell+3*j + 1) > cross_sec_up_down)
				Before_element_draw_flag = 0;
			if( *(coord0_el+3*npell+3*j + 2) > cross_sec_in_out)
				Before_element_draw_flag = 0;
		    }

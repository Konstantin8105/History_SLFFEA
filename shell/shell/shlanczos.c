...
int shConjGrad(double *, BOUND , int *, double *, int *, double *, double *,
	double *, double *, MATL *, double *);

int solve(double *, double *, int *, int );

int shMassPassemble(int *, double *, int *, double *, double *, double *, MATL *,
	double *, double *);

int QR_check( double *, double *, double *, int, int );

int shLanczos(double *K, BOUND bc, int *connect, double *coord, EIGEN *eigen,
...
*/
	    ...
	    check = shMassPassemble(connect, coord, el_matl, lamina_ref,
		fiber_xyz, M, matl, fdum_vector_out, fdum_vector_in);
	    if(!check) printf( " Problems with shMassPassemble \n");
...
*/
		...
		check = shConjGrad( K, bc, connect, coord, el_matl, fdum_vector_in,
			K_diag, lamina_ref, fiber_xyz, matl, fdum_vector_out);

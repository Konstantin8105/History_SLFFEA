...
/* Create the coord -/+*/
                        ...
                        *(coord_el_trans+j) =
				*(coord+*(sdof_el+nsd*j));
                        *(coord_el_trans+npell*1+j) =
				*(coord+*(sdof_el+nsd*j+1));
                        *(coord_el_trans+npell*2+j) =
...
/* for node 0 */
		...
		*(vec_dum1+1) = *(coord_el_trans+npell+1) - *(coord_el_trans+npell);
		*(vec_dum1+2) = *(coord_el_trans+2*npell+1) - *(coord_el_trans+2*npell);

		*(vec_dum2) = *(coord_el_trans+3) - *(coord_el_trans);
		*(vec_dum2+1) = *(coord_el_trans+npell+3) - *(coord_el_trans+npell);
		*(vec_dum2+2) = *(coord_el_trans+2*npell+3) - *(coord_el_trans+2*npell);
...
/* for node 1 */
		...
		*(vec_dum1+1) = *(coord_el_trans+npell+2) - *(coord_el_trans+npell+1);
		*(vec_dum1+2) = *(coord_el_trans+2*npell+2) - *(coord_el_trans+2*npell+1);

		*(vec_dum2) = *(coord_el_trans) - *(coord_el_trans+1);
		*(vec_dum2+1) = *(coord_el_trans+npell) - *(coord_el_trans+npell+1);
		*(vec_dum2+2) = *(coord_el_trans+2*npell) - *(coord_el_trans+2*npell+1);
...
/* for node 2 */
		...
		*(vec_dum1+1) = *(coord_el_trans+npell+3) - *(coord_el_trans+npell+2);
		*(vec_dum1+2) = *(coord_el_trans+2*npell+3) - *(coord_el_trans+2*npell+2);

		*(vec_dum2) = *(coord_el_trans+1) - *(coord_el_trans+2);
		*(vec_dum2+1) = *(coord_el_trans+npell+1) - *(coord_el_trans+npell+2);
		*(vec_dum2+2) = *(coord_el_trans+2*npell+1) - *(coord_el_trans+2*npell+2);
...
/* for node 3 */
		...
		*(vec_dum1+1) = *(coord_el_trans+npell) - *(coord_el_trans+npell+3);
		*(vec_dum1+2) = *(coord_el_trans+2*npell) - *(coord_el_trans+2*npell+3);

		*(vec_dum2) = *(coord_el_trans+2) - *(coord_el_trans+3);
		*(vec_dum2+1) = *(coord_el_trans+npell+2) - *(coord_el_trans+npell+3);
		*(vec_dum2+2) = *(coord_el_trans+2*npell+2) - *(coord_el_trans+2*npell+3);

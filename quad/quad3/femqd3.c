...
int analysis_flag, dof, sdof, EMdof, modal_flag, neqn, EMneqn, nmat, nmode, numed,
...
/* For the doubles */
        sofmf=sdof + 3*dof + EMdof + dof + 2*numel + numnp + numed + 2*EMdof;
	if(modal_flag)
	{
		sofmf = sdof + 3*dof + EMdof + dof + 2*numel + numnp + numed +
...
/* Calculate the number of times a node apprears in the connectivity of the edges.
   This is unlike that of the mechanics elements where node_counter is based on
   element connectivity. */

	    node1 = *(edge_connect+nped*k);
	    node2 = *(edge_connect+nped*k+1);
	    *(node_counter + node1) += 1.0;
	    *(node_counter + node2) += 1.0;
	}

	analysis_flag = 1;
	memset(TOTAL,0,sofmTOTAL*sof);
        check = qd3Kassemble(Att, Btt, Btz, Bzt, Bzz, connect, edge_connect, el_edge_connect,
		coord, el_matl, force, id, idiag, Att_diag, Bzz_diag, lm, matl, edge_counter,
		strain, strain_node, stress, stress_node, EMedge, EMnode, Arean);
...
/* Calculate the reaction forces */
		...
		lm, matl, edge_counter, strain, strain_node, stress, stress_node, EMedge,
		EMnode, Arean);
	    if(!check) printf( " Problems with qd3Kassembler \n");
/*
...
/* Calculate the stresses */
		...
		check = qd3Kassemble(Att, Btt, Btz, Bzt, Bzz, connect, edge_connect,
		    ...
		    Bzz_diag, lm, matl, edge_counter, strain, strain_node, stress,
		    stress_node, EMedge, EMnode, Arean);
		if(!check) printf( " Problems with qd3Kassembler \n"); 

		memset(EMnode,0,dof*sizeof(double));

		for( k = 0; k < numed; ++k )
		{

/* Calculate the value of the global x and y components of the electric field
   for each edge
*/
		    node1 = *(edge_connect+nped*k);
		    node2 = *(edge_connect+nped*k+1);

/* Calculate the value of the electrical field for this Edge */

		    fdum = *(coord+node2*nsd) - *(coord+node1*nsd);
		    fdum2 = *(coord+node2*nsd+1) - *(coord+node1*nsd+1);
		    length = sqrt(fdum*fdum + fdum2*fdum2);

/* Calculate the component of the edge in the x and y directions */

		    fdum /= length;  /* fdum = x component of edge vector */
		    fdum2 /= length;  /* fdum2 = y component of edge vector */

/* Add all the electric field components for a particular node from all the elements which
   share that node */

		    for( i = 0; i < ndof; ++i )
		    {
				*(EMnode + ndof*node1) += fdum*(*(EMedge+k));
				*(EMnode + ndof*node1 + 1) += fdum2*(*(EMedge+k));
				*(EMnode + ndof*node2) += fdum*(*(EMedge+k));
				*(EMnode + ndof*node2 + 1) += fdum2*(*(EMedge+k));
		    }
		}

/*	 Average the nodal electric field values */

		printf( "\n hhhh \n");
		for( i = 0; i < numnp ; ++i )
		{
		   *(EMnode + ndof*i) /= *(node_counter + i);
		   *(EMnode + ndof*i + 1) /= *(node_counter + i);
		   printf( "\n %4d  %16.8e  %16.8e", i, *(EMnode+ndof*i), *(EMnode+ndof*i+1));
		}

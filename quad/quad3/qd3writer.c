...
/* Every output file is named after the input file with
   a ".oqd" extension */
	...
	if(!ccheck) printf( " Problems with strncpy \n");
        ...
        fprintf( o3, "matl no., permeability, permittivity, operating fq \n");
        for( i = 0; i < nmat; ++i )
        {
           fprintf( o3, "  %4d    %12.6e %12.6e %12.6e\n ",i,
		matl[i].eta, matl[i].nu, matl[i].ko);
        }
	...
	printf( "\n");
        ...
        fprintf( o3, "node with electrical field vector in x,y \n");
	...
	if( standard_flag)
	{
		...
		*(id+edof*node+1) = -1;
        ...
        for( i = 0; i < numnp; ++i )
        {
           	...
           	for( j = 0; j < ndof; ++j )
           	{
               		fprintf( o3," %16.4e ",*(EMnode+ndof*i+j));
           	}
           	fprintf( o3, "\n");
        }
        fprintf( o3, " -10\n");
